...
- `bun run build` — runs `build.sh`, emitting a minified `docs/root.js` + `docs/index.html`
...
- Themes are driven by CSS variables in `index.html` plus `theme === "dark"` conditional classes; there is no Tailwind build step (Tailwind loads from the CDN in `index.html`)
